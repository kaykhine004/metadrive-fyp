...
This project develops a simulation-based evaluation framework for three different scenario classes: highway, intersection-roundabout, and street driving with pedestrians. The work started from a practical question rather than a purely theoretical one: if several driving algorithms are tested under the same repeatable conditions, which approach is most suitable for each scenario, and what kind of failure does each approach show? To answer that question, the project implements custom scenario logic, policy assignment, episode logging, workbook export, and notebook-based analysis within a selected three-dimensional simulation platform.

The most important part of the work lies in Chapters 4, 5, and 6, where the system design, implementation logic, and experimental findings are explained in detail. The report shows how the ideas developed through the project process. On the highway, poor progress of one baseline controller motivated a hybrid recovery design. In the intersection-roundabout case, lack of route control motivated a custom fixed-route map and target-specific trajectory assignment. In the street case, the need to evaluate vulnerable road users motivated a multi-crosswalk event pipeline and a pedestrian-aware control extension.

The final results show that performance is strongly scenario-dependent. A simple reactive baseline is the strongest overall reference method in the highway and intersection datasets because it gives the best balance of completion and control stability. In the street scenario, the project-authored pedestrian-aware hybrid controller gives the best balance between route completion and pedestrian safety, although it still needs improvement in vehicle-to-vehicle interaction. The report therefore concludes that scenario-specific engineering is necessary, and that pass rate alone is not enough for safety-oriented evaluation. Timeout behaviour, crash type, speed, and time-to-goal must all be considered together.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{Documentation/SUMO_in_2D_visualization.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\textwidth]{Documentation/CARLA_relealistic.png}
    \caption{Illustrative high-fidelity three-dimensional simulation environment provided by CARLA.}
    \label{fig:carla_platform}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\textwidth]{Documentation/METADrive_realistics.png}
    \caption{Illustrative three-dimensional simulation environment used in the later stage of the project.}
    \label{fig:metadrive_platform}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Illustrative project workflow used during the platform exploration and experiment-development stage.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Documentation/FYP_description.png}
    \caption{Illustrative early-stage simulation-analysis output retained from the original report package.}
    \label{fig:project_workflow_analysis}
\end{figure}
...
The framework was designed to support comparative evaluation across multiple road contexts while still keeping the experiment logic simple enough to trace and modify. Instead of treating the simulator as a general sandbox, the design focused on four concrete objectives:

\begin{enumerate}[label=\arabic*.]
    \item To test whether the relative performance of the same controller family remains consistent when the road context changes significantly.
    \item To examine whether an observed weakness in a baseline policy can be improved through a small and targeted engineering modification.
    \item To separate performance problems caused by scenario design from those caused by controller logic, route definition, or event management.
    \item To use multiple metrics so that completion results can be interpreted together with timeout behaviour, crash burden, speed, and travel time.
\end{enumerate}

These objectives shaped the scenario architecture, the policy comparison strategy, and the final selection of evaluation metrics.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.3cm} >{\raggedright\arraybackslash}p{3.0cm} X}
        \toprule
        \shortstack[l]{Policy or\\component} & \shortstack[l]{Ownership\\status} & Role in the project \\
        \midrule
        ...
        \addlinespace[3pt]
        \shortstack[l]{\texttt{TrajectoryIDM}\\\texttt{Policy}} & MetaDrive baseline & Route-following baseline for comparison \\
        \addlinespace[3pt]
        \shortstack[l]{\texttt{StuckAware}\\\texttt{Expert}\\\texttt{Policy}} & \shortstack[l]{Project-authored\\wrapper} & Highway recovery policy for Expert stagnation \\
        \addlinespace[3pt]
        \shortstack[l]{\texttt{ObstacleAware}\\\texttt{TrajectoryIDM}\\\texttt{Policy}} & \shortstack[l]{Project-authored\\wrapper} & Highway trajectory policy with reactive avoidance mode \\
        \addlinespace[3pt]
        \shortstack[l]{Intersection custom\\map and\\managers} & \shortstack[l]{Project-authored\\implementation} & Fixed-route multi-agent evaluation environment \\
        \addlinespace[3pt]
        \shortstack[l]{Street crosswalk and\\pedestrian event\\pipeline} & \shortstack[l]{Project-authored\\implementation} & Repeated pedestrian interaction testbed \\
        \addlinespace[3pt]
        \shortstack[l]{\texttt{PedestrianAware}\\\texttt{IDM}\\\texttt{Policy}} & \shortstack[l]{Project-authored\\policy\\extension} & Street controller with pedestrian-aware braking and steering logic \\
        \addlinespace[3pt]
        \shortstack[l]{PPO training\\script} & \shortstack[l]{Project-authored\\training\\workflow} & Exploration of learned optimisation on the custom intersection route \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}p{3.7cm} X}
        \toprule
        \shortstack[l]{Observation during\\development} & \shortstack[l]{Design idea\\introduced} & Reasoning \\
        \midrule
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{5.1cm} X}
        ...
        \midrule
        \topstack{\codefile{Final analysis/}\\\codefile{Highway analysis/}\\\codefile{highway_IDM.py,}\\\codefile{highway_Expert+IDM.py,}\\\codefile{highway_Trajectory.py}} & \topstack{Highway environment setup, policy logic,\\and repeated testing} \\
        \addlinespace[4pt]
        \topstack{\codefile{Final analysis/}\\\codefile{Intersection analysis/}\\\codefile{intersection_IDM.py,}\\\codefile{intersection_EXPERT.py,}\\\codefile{intersection_TrajectoryIDM.py}} & \topstack{Custom route environment, target-agent control,\\and intersection evaluation} \\
        \addlinespace[4pt]
        \topstack{\codefile{Final analysis/}\\\codefile{Street analysis/}\\\codefile{street_expert.py,}\\\codefile{street_IDM.py,}\\\codefile{street_HybridIDM.py}} & \topstack{Street scenario generation, crosswalk logic,\\and street policy comparison} \\
        \addlinespace[4pt]
        \topstack{\codefile{backup files/}\\\codefile{train_optimized_policy.py}} & \topstack{PPO training workflow for the custom intersection route} \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{4.4cm} X}
        \toprule
        Scenario & Workbook & Policies and rows used \\
        \midrule
        Highway & \shortstack[l]{\codefile{Highway analysis/}\\\codefile{highway_analysis_}\\\codefile{compilation.xlsx}} & \shortstack[l]{IDM (\texttt{n=50}), Expert (\texttt{n=10}),\\Expert+IDM hybrid (\texttt{n=50}),\\trajectory variant (\texttt{n=10})} \\
        \shortstack[l]{Intersection-\\roundabout} & \shortstack[l]{\codefile{Intersection analysis/}\\\codefile{Intersection_analysis_}\\\codefile{compilation.xlsx}} & \shortstack[l]{IDM (\texttt{n=50}), Expert (\texttt{n=39}),\\TrajectoryIDM (\texttt{n=40})} \\
        Street & \shortstack[l]{\codefile{Street analysis/}\\\codefile{street_analysis_}\\\codefile{compilation.xlsx}} & \shortstack[l]{Expert (\texttt{n=10}), IDM (\texttt{n=10}),\\Hybrid IDM using only rows with\\\texttt{LAP == 1} (\texttt{n=10})} \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{3.2cm} >{\centering\arraybackslash}p{1.6cm} >{\centering\arraybackslash}p{1.8cm} >{\centering\arraybackslash}p{1.9cm} >{\centering\arraybackslash}p{1.8cm} >{\centering\arraybackslash}p{2.1cm}}
        \toprule
        Policy & \shortstack{Pass\\Rate} & \shortstack{Timeout\\Rate} & \shortstack{Avg. Crash\\Count} & \shortstack{Avg. Speed\\(km/h)} & \shortstack{Avg. Goal\\Time (s)} \\
        \midrule
        IDM & 98.0\% & 2.0\% & 1.04 & 24.52 & 164.82 \\
        Expert & 0.0\% & 100.0\% & 1.40 & 3.75 & -- \\
        \shortstack[l]{Expert+IDM\\hybrid} & 82.0\% & 18.0\% & 3.52 & 21.13 & 177.37 \\
        \shortstack[l]{Trajectory+IDM\\variant} & 90.0\% & 10.0\% & 35.40 & 23.00 & 183.22 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{3.2cm} >{\centering\arraybackslash}p{1.6cm} >{\centering\arraybackslash}p{1.8cm} >{\centering\arraybackslash}p{1.9cm} >{\centering\arraybackslash}p{1.8cm} >{\centering\arraybackslash}p{2.1cm}}
        ...
        Policy & \shortstack{Pass\\Rate} & \shortstack{Timeout\\Rate} & \shortstack{Avg. Crash\\Count} & \shortstack{Avg. Speed\\(km/h)} & \shortstack{Avg. Goal\\Time (s)} \\
        \midrule
        IDM & 100.0\% & 0.0\% & 2.82 & 26.10 & 50.60 \\
        Expert & 66.7\% & 23.1\% & 1.28 & 16.26 & 59.94 \\
        TrajectoryIDM & 90.0\% & 0.0\% & 5.10 & 28.35 & 53.87 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{3.2cm} >{\centering\arraybackslash}p{1.6cm} >{\centering\arraybackslash}p{1.8cm} >{\centering\arraybackslash}p{1.9cm} >{\centering\arraybackslash}p{1.9cm} >{\centering\arraybackslash}p{2.1cm}}
        \toprule
        Policy & \shortstack{Pass\\Rate} & \shortstack{Timeout\\Rate} & \shortstack{Mean Ped.\\Crashes} & \shortstack{Mean Veh.\\Crashes} & \shortstack{Avg. Goal\\Time (s)} \\
        \midrule
        Expert & 90.0\% & 10.0\% & 1.80 & 1.00 & 140.26 \\
        IDM & 0.0\% & 100.0\% & 0.00 & 0.00 & -- \\
        \shortstack[l]{Hybrid IDM\\(LAP 1)} & 100.0\% & 0.0\% & 0.20 & 4.20 & 148.71 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{HIghway_idm_passed.png}
    \caption{Representative highway result illustrating the strong completion performance of the IDM policy.}
    \label{fig:highway_idm_example}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Documentation/Highway_expert_failed.png}
    \caption{Representative highway failure case showing repeated timeout behaviour for the Expert policy.}
    \label{fig:highway_expert_failure}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{IDM_trajectory_Many_crashes.png}
    \caption{Representative highway result showing heavy crash accumulation for the trajectory-based policy variant.}
    \label{fig:highway_trajectory_failure}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Intersection_chart.png}
    \caption{Representative intersection-roundabout comparison illustrating the trade-off between completion and crash burden.}
    \label{fig:intersection_detail_chart}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{ExpertPolicy_crashes_instreet.png}
    \caption{Representative street result showing the high pedestrian-crash burden of the Expert policy.}
    \label{fig:street_expert_failure}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{IDM_Policy_highTImeout_Street.png}
    \caption{Representative street result showing the severe timeout behaviour of the IDM policy.}
    \label{fig:street_idm_timeout}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{HybidIDM_goodSuccessRate_LessTimetaken_street.png}
    \caption{Representative street result showing the strong completion performance of the Hybrid IDM controller.}
    \label{fig:street_hybrid_success}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.7cm} >{\raggedright\arraybackslash}p{2.6cm} X}
        \toprule
...
\begin{enumerate}[label=\arabic*.]
    \item Upload \codefile{main.tex} to Overleaf as the main report source.
    \item Upload \codefile{Final_Project_Report_References.bib} beside the main \texttt{.tex} file.
    \item Upload the folder \codefile{Documentation/} so that the existing figure paths remain valid.
    \item Upload \codefile{NTU logo.png} in the same location as the main \texttt{.tex} file.
...
The figure filenames and paths in this report have been kept in their original form.
