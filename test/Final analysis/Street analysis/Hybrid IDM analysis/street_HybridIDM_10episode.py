...
10-episode batch: logs Pass / Timeout / crashes / time / speed to an .xlsx in
``Hybrid IDM analysis`` (see OUTPUT_DIR).
...
STOP_SPEED_KMH = 0.5
POLICY_DISPLAY_NAME = "Hybrid IDM (PedestrianAwareIDMPolicy)"
OUTPUT_DIR = (
    r"C:\Users\User\Desktop\FYP\metadrive0305\metadrive-main\test\Final analysis"
    r"\Street analysis\Hybrid IDM analysis"
)


def _save_results_xlsx(rows: list[dict]) -> str:
    """Write one row per episode; file is always under OUTPUT_DIR."""
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    ts = time.strftime("%Y%m%d_%H%M%S")
    path = os.path.join(OUTPUT_DIR, f"street_HybridIDM_results_{ts}.xlsx")
    wb = Workbook()
    ws = wb.active
    ws.title = "HybridIDM"
    # Column order matches FYP spreadsheet: episode, pass, timeout, ped/veh crashes, goal time, avg speed, timestamp
    ws.append(
        [
            "Episode",
            "Pass",
            "Timeout",
            "crash_count_pedestrian_with_my_car",
            "crash_count_with_other_vehicles",
            "time_taken_to_reach_goal_sim_s",
            "speed_avg_kmh",
            "date_and_time_of_simulation",
        ]
    )
    for r in rows:
        ws.append(
            [
                ...
                r["Pass"],
                r["Timeout"],
                r["crash_count_pedestrian_with_my_car"],
                r["crash_count_with_other_vehicles"],
                r["time_taken_to_reach_goal_sim_s"],
                r["speed_avg_kmh"],
                r["date_and_time_of_simulation"],
            ]
        )
    wb.save(path)
    return path
...
def main():
    ...
    try:
        ...
        for episode_num in range(1, TOTAL_EPISODES + 1):
                ...
                if use_render:
                    env.render(
                        text={
                            ...
                            "1. Policy": POLICY_DISPLAY_NAME,
                            "2. Wall time": f"{wall_s:.2f} s",
                            "3. Sim time": f"{sim_elapsed:.1f} s",
                            "4. Timeout": "YES" if timeout else "NO",
                            "5. Ped crashes": str(ped_hit_count),
                            "6. Veh crashes": str(veh_crash_count),
                            "7. Route %": f"{route_pct:.1f}%",
                        }
                    )
            ...
            pass_ok = reached_goal and not timeout
            results.append(
                {
                    "Episode": episode_num,
                    "Pass": "YES" if pass_ok else "NO",
                    "Timeout": "YES" if timeout else "NO",
                    "crash_count_pedestrian_with_my_car": ped_hit_count,
                    "crash_count_with_other_vehicles": veh_crash_count,
                    "time_taken_to_reach_goal_sim_s": round(sim_elapsed, 2) if reached_goal else "N/A",
                    "speed_avg_kmh": round(speed_avg, 2),
                    "date_and_time_of_simulation": sim_datetime,
                }
            )
            print(
