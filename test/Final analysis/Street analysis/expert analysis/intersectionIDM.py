#!/usr/bin/env python
"""
Roundabout + Intersection — 10 continuous episodes, IDMPolicy.

Tracks **one vehicle only** (TARGET_AGENT). Other agents are ignored for logging.

Rules (per episode):
- Crashes: counted on **rising edge** (each new contact event). Counters reset every episode.
- **Timeout**: ego speed ≤ STOP_SPEED_KMH for ≥ STOP_TIMEOUT_SEC (simulation time) → episode ends,
  Timeout = YES.
- **Pass**: YES if Timeout = NO, else NO (per spec).
- Press **F** once at start (pyautogui) for MetaDrive fast simulation mode.

Results: one .xlsx after all 10 episodes.
"""
    ...
    for row in rows:
        ws.append(
            [
                ...
                row["Pass"],
                row["Timeout"],
                row["crash_count_pedestrian"],
                row["crash_count_vehicle"],
...
def _set_vehicle_visuals_and_track(env, target_id):
    for agent_id, vehicle in env.agents.items():
        if agent_id == target_id:
            vehicle.origin.setColorScale(1.0, 0.05, 0.05, 1.0)
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        for episode in range(1, TOTAL_EPISODES + 1):
            ...
            start_pos = (
                tuple(env.agents[TARGET_AGENT].position)
            ...
            for _ in range(1, 10_000_000):
                ...
                tin = info.get(TARGET_AGENT, {})
                ch = bool(tin.get("crash_human", False))
                cv = bool(tin.get("crash_vehicle", False))
                ...
                hud = {
                    ...
                    "Tracked (only)": f"RED = {TARGET_AGENT}",
                    "Policy": "IDMPolicy",
                    "Speedup": "FAST (F)" if fast_mode_enabled else "NORMAL",
                    "Sim time (s)": f"{sim_elapsed_sec:.1f}",
                    "Timeout": "YES" if timeout else "NO",
                    "Pass (spec)": "YES" if not timeout else "NO",
                    "Crash ped": crash_ped,
                    "Crash vehicles": crash_veh,
                }
                env.render(text=hud)
                ...
                if td_img is not None:
                    tracked = env.agents.get(TARGET_AGENT, None)
                    vehicle_pos = tuple(tracked.position) if tracked is not None else start_pos
            ...
            print(
                f"Episode {episode} | Pass:{'YES' if not timeout else 'NO'} | "
                f"Timeout:{timeout} | ped_crash:{crash_ped} veh_crash:{crash_veh} | "
                f"goal:{reached_goal} | sim_t:{episode_elapsed:.1f}s"
            )
