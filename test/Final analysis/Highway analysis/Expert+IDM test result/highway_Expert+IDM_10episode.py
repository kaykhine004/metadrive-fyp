#!/usr/bin/env python
"""
highway_Expert+IDM.py
Runs 10 consecutive episodes with StuckAwareExpertPolicy (Expert PPO + IDM fallback)
on the highway scenario.

Per-episode tracking:
  - Crash count        : increments each time a new collision event begins.
  - Timeout            : YES if the agent is stuck (moved < 5m in 60s).
  - Pass               : YES if no timeout occurred, NO otherwise.
  - Time Taken         : adaptive timer (simulation time when F pressed).
  - Avg Speed (km/h)   : mean of per-step speeds.

Results saved to XLSX in Expert+IDM test result folder.
"""
import math
import os
import time
...
STUCK_DIST_M         = 5.0    # metres — min displacement to NOT be stuck
STUCK_WINDOW_SEC     = 60.0   # seconds — rolling window for stuck detection
AUTO_PRESS_F         = True   # auto-enable unlimited FPS on episode 1

RESULT_DIR = (
    r"C:\Users\User\Desktop\FYP\metadrive0305\metadrive-main"
    r"\test\Final analysis\Highway analysis\Expert+IDM test result"
)
...
def save_results_xlsx(results: list, start_dt: datetime):
    ...
    filename = f"Expert+IDM_results_{start_dt.strftime('%Y%m%d_%H%M%S')}.xlsx"
