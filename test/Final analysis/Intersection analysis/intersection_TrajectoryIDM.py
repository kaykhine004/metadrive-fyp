#!/usr/bin/env python
"""
Roundabout + Intersection single-route simulation driven by TrajectoryIDMPolicy (agent0) + IDMPolicy (others).

TrajectoryIDMPolicy: agent0 follows a pre-built PointLane trajectory with IDM speed control.
Other agents use standard IDMPolicy for routing-based driving.
...
MA_CONFIG = dict(
    ...
    crash_done=False,
    allow_respawn=False,
    agent_policy=TrajectoryIDMPolicy,  # target agent uses TrajectoryIDM; others use IDM (see TrajectoryIDMAgentManager)
    ...
    try:
        ...
        for i in range(1, 10_000_000):
            ...
            if target_done:
                ...
                print(f"  Policy: TrajectoryIDMPolicy")
