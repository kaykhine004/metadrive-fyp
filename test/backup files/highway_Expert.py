...
# Path to the weights saved by highway_ExpertTrained.py
_THIS_DIR       = os.path.dirname(os.path.abspath(__file__))
TRAINED_MODEL   = os.path.join(_THIS_DIR, "trained_expert", "ppo_highway_expert")
...
if __name__ == "__main__":
    # Load the trained PPO weights saved by highway_ExpertTrained.py
    if not os.path.exists(TRAINED_MODEL + ".zip"):
        raise FileNotFoundError(
            f"Trained model not found at {TRAINED_MODEL}.zip\n"
            "Run highway_ExpertTrained.py first to generate the weights."
        )
    model = PPO.load(TRAINED_MODEL)
    print(f"Loaded trained model from {TRAINED_MODEL}.zip")

    env = SafeMetaDriveEnv(
        dict(
            use_render=True,
            manual_control=False,
            # No agent_policy — actions come from model.predict() below
            map="CSrRSY$yRSCR",
            num_scenarios=1,
            start_seed=5,
            ...
            out_of_road_done=True,           # reset episode immediately when off-road
            on_continuous_line_done=False,   # don't end on line crossings (ramps/toll)
    ...
    try:
        ...
        for i in range(1, 1_000_000_000):
            ...
            obs = np.nan_to_num(o, nan=0.0, posinf=1.0, neginf=-1.0)

            crash_now   = bool(info.get("crash_vehicle", False) or info.get("crash_object", False))
            off_road    = bool(info.get("out_of_road", False))
            route_pct   = float(info.get("route_completion", 0.0)) * 100.0
            ...
            env.render(
                text={
                    ...
                    "Route completion": f"{route_pct:.1f}%",
                    "Off-road": "YES (resetting)" if off_road else "no",
                    "Speed up": "Press F",
                }
            )
