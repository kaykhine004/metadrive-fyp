#!/usr/bin/env python
"""
highway_expert10episode.py
Runs 10 consecutive episodes with ExpertPolicy (PPO) on the highway scenario.
...
class StuckAwareExpertPolicy(ExpertPolicy):
    """
    ExpertPolicy that falls back to IDMPolicy when the agent gets stuck in front
    of an obstacle. Uses position-based stuck detection: if the vehicle moves
    less than STUCK_AVOID_DIST_M within STUCK_AVOID_WINDOW_STEPS steps, switch
    to IDM which can lane-change to avoid static obstacles.
    """
    STUCK_AVOID_DIST_M = 3.0      # min displacement to NOT be considered stuck
    STUCK_AVOID_WINDOW_STEPS = 50  # ~5 sec at 0.1s/step — check window
    STUCK_SPEED_KMH = 2.0         # speed below this = likely stuck

    def __init__(self, control_object, random_seed=None, config=None):
        super(StuckAwareExpertPolicy, self).__init__(control_object, random_seed, config)
    ...
    def _get_fallback_policy(self):
        if self._fallback_policy is None:
            ...
            logger.warning("Stuck — falling back to IDMPolicy for this episode.")
    ...
    def act(self, agent_id=None):
        ...
        # Stuck detection: switch to IDM when not making progress
        try:
            ...
            steps_in_window = step - self._stuck_ref_step
            if steps_in_window >= self.STUCK_AVOID_WINDOW_STEPS:
                dist_moved = float(np.linalg.norm(cur_pos - self._stuck_ref_pos))
                if dist_moved < self.STUCK_AVOID_DIST_M or (
                    steps_in_window >= 30 and speed_kmh < self.STUCK_SPEED_KMH
                ):
                    self._use_fallback = True
                    logger.warning(
                        "Stuck detected (moved %.1fm in %d steps, speed %.1f km/h). "
                        "Switching to IDM to avoid obstacle.",
                        dist_moved, steps_in_window, speed_kmh
                    )
                    return self._get_fallback_policy().act(agent_id)
        ...
        return super(StuckAwareExpertPolicy, self).act(agent_id)

    def reset(self):
        super(StuckAwareExpertPolicy, self).reset()
        self._stuck_ref_pos = None
        self._stuck_ref_step = 0
...
RESULT_DIR = (
    ...
    r"\test\Final analysis\Highway analysis\Expert test result"
)
...
def save_results_xlsx(results: list, start_dt: datetime):
    ...
    filename = f"Expert_results_{start_dt.strftime('%Y%m%d_%H%M%S')}.xlsx"
