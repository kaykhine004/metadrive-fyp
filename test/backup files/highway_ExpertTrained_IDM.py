#!/usr/bin/env python
"""
Highway simulation using a combined Expert + IDM fallback policy.

- Primary: PPO Expert (trained on standard roads) — smooth, fast, avoids vehicles/obstacles.
- Fallback: IDMPolicy — automatically activated when Expert fails (toll gate, obs mismatch).
- The policy resets to Expert at the start of every new episode.

The 2D top-down road map overlay (top-left corner) shows START (blue) and END (red) markers
with a live green arrow tracking the agent. The HUD shows which policy is currently active.
"""
import math
...
if __name__ == "__main__":
    env = SafeMetaDriveEnv(
        dict(
            ...
            # Stable route: ramps + split/merge + tollgate.
            map="CSrRSY$yRSCR",
            num_scenarios=1,
            start_seed=5,
            # Same traffic/obstacle settings as highway_IDM.py for consistent comparison.
            traffic_density=0.05,
            need_inverse_traffic=True,
            random_traffic=True,
            # Construction/accident events at some map blocks (identical to highway_IDM.py).
            accident_prob=0.25,
    ...
    try:
        ...
        for i in range(1, 1_000_000_000):
            ...
            active_policy = "IDM (fallback)" if (isinstance(policy, ExpertPolicy) and policy._use_fallback) else "Expert (PPO)"

            env.render(
                text={
                    "Policy": "Expert + IDM fallback",
