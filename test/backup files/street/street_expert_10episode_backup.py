#!/usr/bin/env python
"""
Street expert — 10 episodes with logging.

- Runs 10 episodes back-to-back on the same procedural map (ExpertPolicy).
- Mid-route pedestrian crossing on the ego lane (1–2 peds/episode); crosswalks on.
- Per episode: crash count (edge-triggered), stall timeout (>1 min simulated time nearly stopped).
- Pass = YES if no timeout; Timeout = YES/NO.
- Presses F once after startup (via pyautogui) to toggle unlimited FPS when possible.
- After 10 episodes, writes an .xlsx to: .../expert analysis/
- On-screen `sim_time_s` is simulated seconds for the current episode only (resets every `env.reset`).

Requires: pip install openpyxl pyautogui

Env (optional):
  METADRIVE_HEADLESS=1  — no window / no render (skips pyautogui F).
  STREET_EXPERT_EPISODES=N  — default 10; number of consecutive episodes before saving one xlsx.

Dynamic mid-route pedestrians are removed before each env.reset() so multi-episode runs do not assert.
"""
...
OUTPUT_DIR = Path(
    ...
    r"\test\Final analysis\Street analysis\expert analysis"
)

NUM_EPISODES = int(os.environ.get("STREET_EXPERT_EPISODES", "10"))
STALL_SPEED_KMH = 1.0
STALL_SECONDS = 60.0

# Mid-route pedestrian crossing (ego lane when progress hits mid band; graph fallback)
...
PEDESTRIAN_COUNT_RANGE = (1, 2)
MAX_CROSSING_CANDIDATES = 8
MIN_SPAWN_SEPARATION = 2.5


class MidRouteCrossingController:
    """Crossing placed on ego lane when route progress enters the mid band; 1–2 peds/episode."""
    ...
    def cleanup_spawned_pedestrians(self, env):
        """Must run before env.reset(): clears dynamic peds from the physics world."""
...
def _press_fps_unlock():
    """MetaDrive: F toggles unlimited / realtime FPS."""
...
def _crash_edge(prev_flags, info):
    """Return True if a crash flag just became True."""
...
def _sim_seconds_per_env_step(env: MetaDriveEnv) -> float:
    """
    Simulated seconds advanced by one call to env.step().

    Each env.step() runs `decision_repeat` Bullet substeps of length
    `physics_world_step_size`. Use engine.global_config so this matches the
    actual physics integration (not a stale config copy).
    """
...
def main():
    ...
    try:
        ...
        print(HELP_MESSAGE)
        print(f"Running {NUM_EPISODES} episodes. Results -> {OUTPUT_DIR}")
        print("Mid-route: 1–2 pedestrians cross the ego street when you reach ~38–58% route progress.")
        ...
        for episode in range(1, NUM_EPISODES + 1):
            ...
            print(
                f"\n>>> Episode {episode}/{NUM_EPISODES} start (sim time reset to 0 s)\n",
                flush=True,
            )
            ...
            while True:
                ...
                last_info = info
                num_env_steps += 1
                # Sim time = (# of env.step calls) × dt per step (do not rely on episode_step alone;
                # keep in sync with stall timeout integration).
        ...
        out_path = OUTPUT_DIR / f"expert_10episode_{ts}.xlsx"
