...
Policy: OptimizedPolicy (PPO-trained) for agent0, IDMPolicy for others
...
from metadrive.policy.optimized_policy import OptimizedPolicy
...
# ---------------------------------------------------------------------------
# Agent manager — OptimizedPolicy for agent0, IDMPolicy for others
# ---------------------------------------------------------------------------
class OptimizedAgentManager(VehicleAgentManager):
...
class MultiAgentRoundaboutIntersectionEnv(MultiAgentMetaDrive):
    ...
    def _get_agent_manager(self):
        return OptimizedAgentManager(init_observations=self._get_observations())
...
def _save_xlsx(results):
    ...
    path = os.path.join(OUTPUT_DIR, f"intersection_Optimized_results_{ts}.xlsx")
    wb = Workbook()
    ws = wb.active
    ws.title = "Results"
    ws.append([
    ...
    for row in results:
        ws.append([
            row["episode"], row["Pass"], row["Timeout"], row["crash_count"],
            row["time_taken_to_reach_goal_sec"], row["speed_avg_kmh"], row["simulation_datetime"],
        ])
    wb.save(path)
    return path
                ...
                if td_img is not None:
                    ...
                    if overlay is None:
                        overlay = OnscreenImage(
                            image=td_tex, pos=(-0.62, 0, 0.55),
                            scale=(0.40, 1, 0.40), parent=env.engine.aspect2d,
                        )
